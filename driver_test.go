...
func startDestroyInspectImage(t *testing.T, image string, taskName string) {
	...
	imageID, err := getPodmanDriver(t, d).createImage(image, &AuthConfig{}, false, 5*time.Minute, false, task)
	require.NoError(t, err)
...
func createInspectImage(t *testing.T, image, reference string) {
	...
	idTest, err := getPodmanDriver(t, d).createImage(image, &AuthConfig{}, false, 5*time.Minute, false, task)
	require.NoError(t, err)
